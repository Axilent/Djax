...
class Recipient(models.Model):
    ...
    def inbox(self,*message_models,**filters):
        ...
        if message_models:
            messages = messages.filter(local_content_type__pk__in=[ct.pk for ct in ContentType.objects.get_for_models(*message_models)])
...
class MessageManager(models.Manager):
    ...
    def create_message(self,message_model,sender):
        ...
        for key,value in message_model.Axilent.field_map.items():
            content[value] = unicode(getattr(message_model,key))
